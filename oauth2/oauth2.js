...
OAuth2.prototype.openAuthorizationCodePopup = function(callback)
{
    ...
    chrome.tabs.create({url: this.adapter.authorizationCodeURL(this.getConfig())},
        function(createdTab)
        {
            ...
            {
                if(tabId === createdTab.id && "complete" === changeInfo.status && 0 < tab.title.search("code"))
                {
                    var windowTitle = tab.title;
                    chrome.tabs.remove(tabId, function()
                    {
                        b.finishAuth(windowTitle, function(){
                            callback();
                        })
                    })
                }
...
OAuth2.prototype.getAuthorizationHeader = function(a) {
    this.authorize(function() {
        a("Bearer " + this.get('accessToken'))
    })
};
